...
from lte.zcsequence import zcsequence_f, zcsequence_t
from lte.helpers import corr, fshift, tfft, itfft, with_sample_offset, NFFT, MAXNCARRIERS, NCARRIERS, MAXNCARRIERS_c2, \
    NCARRIERS_c2, CP_LENGTHS_legacy, ZC_SYMBOL_IDX_legacy, CP_LENGTHS, CP_LENGTHS_C2, ZC_SYMBOL_IDX, ZC_SYMBOL_IDX_c2


class Packet:
    """Demodulate frames from raw samples to QPSK data"""

    def __init__(self, raw_samples, Fs=15.36e6, enable_zc_detection=True, legacy=False,
                 packet_type="droneid"):
        ...
        self.symbols_time_domain, self.symbols_freq_domain = self.raw_data_to_symbols(self.raw_samples_orig, self.start,
                                                                                      ffo=self.detected_ffo)
        ...
        self.symbols_time_domain, self.symbols_freq_domain = self.raw_data_to_symbols(self.raw_samples, self.start,
                                                                                      ffo=self.detected_ffo,
    ...
    def raw_data_to_symbols(self, samples, first_symbol_offset, ffo=None, sampling_offset=None, angle=None,
                            linear_rotation=None):
    ...
    def get_symbol_data(self, linear_rotation=0, _sampling_offset=0, tune=0, skip_zc=False):
        ...
        # all symbols
        _, all_symbols_f = self.raw_data_to_symbols(self.raw_samples_orig, self.start, ffo=ffo,
                                                    sampling_offset=sampling_offset, linear_rotation=linear_rotation)
